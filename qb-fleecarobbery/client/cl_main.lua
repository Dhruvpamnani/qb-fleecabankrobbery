...
RegisterNetEvent("qb-fleeca:startLoot_b")
AddEventHandler("qb-fleeca:startLoot_b", function(data, name)
    currentname = name
                    ...
                    if not LootCheck[name].Loot then
                        ...
                        if dst1 < 2 then
                            if not helpTextShowing then
                               -- exports['qb-ui']:DrawTextUi('show', "[E] Grab it!")
                               exports['qb-drawtext']:DrawText('[E] Grab it!','left')	
                                helpTextShowing = true
                            elseif dst1 < 0.75 and IsControlJustReleased(0, 38) then
                               -- exports['qb-ui']:HideTextUi('hide')
...
RegisterNetEvent('qb-fleeca:uselaptop3')
AddEventHandler('qb-fleeca:uselaptop3', function(item)
    ...
    for k, v in pairs(Fleeca.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                QBCore.Functions.TriggerCallback('qb-fleeca:server:getCops', function(cops)
                    if cops >= 0 then
                        ...
                        if HackingFleeca(60000, 3) then
                            ...
                            DeleteObject(laptop)
                            ClearPedSecondaryTask(ply)
                            Citizen.Wait(240000) -- 4 min 240000 ms
